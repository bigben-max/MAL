...
#define ADEBUG_MODULE(module)                                        \
  VLOG(4) << LEFT_BRACKET << module << RIGHT_BRACKET << LEFT_BRACKET \
          << __FUNCTION__ << RIGHT_BRACKET << "[DEBUG] "
...
#define ALOG_MODULE_STREAM_INFO(module)                          \
  google::LogMessage(__FILE__, __LINE__, google::INFO).stream()  \
      << LEFT_BRACKET << module << RIGHT_BRACKET << LEFT_BRACKET \
      << __FUNCTION__ << RIGHT_BRACKET
...
#define ACHECK(cond)                                                          \
  CHECK(cond) << LEFT_BRACKET << MODULE_NAME << RIGHT_BRACKET << LEFT_BRACKET \
              << __FUNCTION__ << RIGHT_BRACKET

#define AINFO_EVERY(freq)                                                 \
  LOG_EVERY_N(INFO, freq) << LEFT_BRACKET << MODULE_NAME << RIGHT_BRACKET \
                          << LEFT_BRACKET << __FUNCTION__ << RIGHT_BRACKET
